#pragma once
#include <Arduino.h>

class Timeout
{
public:
    void prepare(uint32_t duration); // setup duration but don't start -> time_over = 1
    void start(uint32_t duration);   // setup time and start the timer
    bool time_over();                // return true if time ran out without reset
    void reset();                    // reset the timer
    void expire();                   // wind the timer forward so time_over is true
    bool periodic();                 // returns a single true when time runs out and
                                     // then resets itself

private:
    bool force_timeout_ = true;
    uint32_t duration_ = 0;
    uint32_t last_reset_ = 0;
};
